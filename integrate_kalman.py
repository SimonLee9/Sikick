...
x_hat = np.array([0.0, 0.0])  # Initial state estimate [x, y]
P = np.eye(2)  # Initial error covariance matrix
Q = np.eye(2) * 0.01  # Process noise covariance matrix (adjust as needed)
R = np.eye(2) * 0.1  # Measurement noise covariance matrix (adjust as needed)
...
def callback5(msg):
    ...
    if not rtk_signal_strong:
        ...
        B = np.zeros(2)  # Control input matrix (not used here)
        u = np.zeros(2)  # Control input (not used here)
        ...
        H = np.eye(2)  # Measurement matrix (identity for direct measurement)
        #z = np.array([zed2_x, zed2_y])  # Measurement vector
        z = np.array([utm_x, utm_y])  # Measurement vector
