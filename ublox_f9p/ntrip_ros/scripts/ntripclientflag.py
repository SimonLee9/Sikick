...
class ntripconnect(Thread):
    ...
    def run(self):
        ...
        while not self.stop:
            # weak signal flag	    
            if self.is_weak_signal(response):
		print("weak signal")
                self.weak_signal = True
                # ROS with weak signal flag
                self.publish_weak_signal_flag()
            else:
		print("RTK signal")
                self.weak_signal = False
    ...
    def is_weak_signal(self, response):
        return len(response.read(0)) < 10 #YOUR_SIGNAL_STRENGTH_THRESHOLD

    def publish_weak_signal_flag(self):
        pass
