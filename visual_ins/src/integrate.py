...
def Current_Odom_publisher(event):
    ...
    if rtk_signal_strong == False: # Get data from ZED2 Camera
        ...
        Before_x = zed2_odom.pose.pose.position.x
        Before_y = zed2_odom.pose.pose.position.y
        
        if zed2_odom:


            now_x = zed2_odom.pose.pose.position.x
            now_y = zed2_odom.pose.pose.position.y

            delta_x = now_x - Before_x
            delta_y = now_y - Before_y

            #delta_x = zed2_odom.pose.pose.position.x
            #delta_y = zed2_odom.pose.pose.position.y
